...
async function GetTopArtist(range) {
    ...
    return (
        <div className=" h-96 font-Inter bg-[#80808005] pl-6 pr-4 pt-2 pb-5 rounded-md shadow-md shadow-black backdrop-blur border-[#ffffff27] border flex flex-col items-center justify-center">
        <h2 className="text-xl font-bold pb-5 fixed top-2"> Top Artists</h2>
        ...
        {artists.items.map((item) =>(
            <li key={item.id} className="list-decimal list-outside font-bold text-sm mx-2">
                <img className="object-cover h-20 w-20 rounded-md " src={item.images[2].url} />
                <p className="text-xs leading-none font-light pr-4 w-24 h-8">{item.name}</p>
             </li>
        ))}
        </ul>
    </div>
